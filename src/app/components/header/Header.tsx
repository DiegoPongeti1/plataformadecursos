...
export function Header() {
    ...
    useEffect(() => { // executa quando o componente for montado
    ...
    useEffect(() => { // usa a tecla esc para fechar o menu lateral
        ...
        return () => window.removeEventListener('keydown', handle); // executa quando o componente for desmontado
    }, [])

    return (
        <>

            <nav className="flex items-center gap-6 justify-start md:justify-center bg-primary py-2 sm:py-4 px-6 fixed w-full ">
                ...
                <ul className="flex gap-4 items-center " tabIndex={drawer ? -1 : undefined}>
                    ...
                    <li className="hidden sm:block">
                        <Link href="https://blog.codarse.com" target="_blank" className="flex items-center gap-1 outline-offset-4">
                            Blog
                            <MdOpenInNew />
                        </Link>
                    </li>
                ...
                <div data-open={drawer}
                    ...
                    className="sm:hidden bg-gradient-to-r from-background fixed top-0 left-0 bottom-0 right-0  transition-transform duration-50  data-[open=false]:-translate-x-full sm:hidden"
